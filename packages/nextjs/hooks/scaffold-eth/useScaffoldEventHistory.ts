...
>({
  ...
  const isContractAddressAndClientReady =
    (Boolean(contractAddress) || Boolean(deployedContractData?.address)) && Boolean(publicClient);
  ...
  const query = useInfiniteQuery({
    ...
    queryFn: async ({ pageParam }) => {
      ...
      const data = await getEvents(
        { address: contractAddress || deployedContractData?.address, event, fromBlock: pageParam, args: filters },
        publicClient,
