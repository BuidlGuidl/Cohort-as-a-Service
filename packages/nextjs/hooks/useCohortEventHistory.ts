 ...
 */
export const useCohortEventHistory = <
...
>({
  ...
  const { data: deployedContractData } = useLocalDeployedContractInfo({
    contractName,
  });
  ...
  const isContractAddressAndClientReady = Boolean(contractAddress) && Boolean(publicClient);
  ...
  const query = useInfiniteQuery({
    ...
    queryFn: async ({ pageParam }) => {
      ...
      const data = await getEvents(
        { address: contractAddress, event, fromBlock: pageParam, args: filters },
        publicClient,
