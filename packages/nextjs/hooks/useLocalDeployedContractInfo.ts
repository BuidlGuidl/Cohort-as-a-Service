 ...
 */
export function useLocalDeployedContractInfo<TContractName extends ContractName>(
  config: UseDeployedContractConfig<TContractName>,
): DeployedContractData<TContractName>;
/**
 * @deprecated Use object parameter version instead: useLocalDeployedContractInfo({ contractName: "YourContract" })
 */
export function useLocalDeployedContractInfo<TContractName extends ContractName>(
...
export function useLocalDeployedContractInfo<TContractName extends ContractName>(
  ...
  useEffect(() => {
    if (typeof configOrName === "string") {
      console.warn(
        "Using `useLocalDeployedContractInfo` with a string parameter is deprecated. Please use the object parameter version instead.",
      );
    }
  }, [configOrName]);
  const { contractName } = finalConfig;
  const selectedNetwork = useSelectedNetwork(baseChainId);
