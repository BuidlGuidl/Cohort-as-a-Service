...
interface UseToggleAllowApplicationsProps {
  cohortAddress: string;
  allowApplications: boolean;
}

export const useToggleAllowApplications = ({ cohortAddress, allowApplications }: UseToggleAllowApplicationsProps) => {
      ...
      try {
        const makeWriteWithParams = () =>
          writeContractAsync({
            ...
            functionName: "toggleAllowApplications",
            args: [!allowApplications],
          });
  ...
  return {
    toggleAllowApplications: sendContractWriteTx,
    isPending,
    isSuccess,
  };
};
